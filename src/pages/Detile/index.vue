            ...
            <div class="cartWrap">
              <div class="controls">
                ...
                <input
                  ...
                  class="itxt"
                  v-model="skuNub"
                  @change="skuNub = skuNub >= 1 ? parseInt(skuNub) : 1"
                />
              ...
              <div class="add">
                <a href="javascript:" @click="goShopChat">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
import {mapGetters} from 'vuex'
  ...
  export default {
    ...
    methods:{
      // 在使用dispatch的时候就是在调用actions中对应的方法,也可以接受一个返回值,
      //async会返回一个promise,状态收到函数return的结果影响
      // 如果需要短暂存储我们需要用到sessStorage,长久保存用到localStorage
      async goShopChat(){
        const {skuNub,skuid} = this
          try {
            await this.$store.dispatch('postShopChat',{skuid,skuNub})
          ...
          type: 'success'
            ...
            sessionStorage.setItem('SET_SHOP_CHAT',JSON.stringify(this.skuInfo))
            this.$router.push({name:'shopchat',query:{skuid}})
          } catch (error) {
              alert(error)
          }
      },
      // 排他思想,添加高亮效果
      spuSaleAttrShow(oneAttrValue,ValueList){
        // console.log(oneAttrValue,ValueList);
        ValueList.forEach(valueObj => {
            valueObj.isChecked = '0'
        });
        oneAttrValue.isChecked = '1'
      },
      // 派发事件
      getGoodsInfo(){
        this.$store.dispatch('getDetileInfo',this.skuid)
      }
    },
    components: {
      ImageList,
